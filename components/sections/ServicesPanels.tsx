    ...
    <section className="relative bg-ink-2 px-6 py-28 md:px-10 md:py-36" id="services">
      <div className="mx-auto max-w-6xl">
        <motion.p
          ...
          className="mb-3 font-display text-xs font-semibold uppercase tracking-[0.5em] text-accent-2"
        >
          What We Fix
        </motion.p>
        <motion.h2
          ...
          <div className="flex flex-col gap-1">
            {business.services.map((service) => (
              <button
                ...
                className={`group flex items-baseline justify-between border-b border-line py-5 text-left transition-colors ${
                  active === service.id ? "text-paper" : "text-mute hover:text-paper/70"
                }`}
              >
                <span className="font-display text-2xl font-bold md:text-3xl">{service.title}</span>
                <span
                  className={`font-display text-xs uppercase tracking-widest transition-opacity ${
                    active === service.id ? "opacity-100 text-accent-2" : "opacity-0"
                  }`}
                >
                  View
                </span>

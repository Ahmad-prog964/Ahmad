    ...
    <section className="relative bg-ink px-6 py-28 md:px-10 md:py-36">
      <div className="mx-auto max-w-4xl text-center">
        ...
        >
          ...
          <p className="mt-4 font-display text-5xl font-extrabold text-paper md:text-7xl">
        ...
        >
          {business.reviewTags.map((tag) => (
            <span
              key={tag.label}
              className="rounded-full border border-paper/15 px-4 py-2 text-xs uppercase tracking-[0.15em] text-paper/80"
            >
              {tag.label} <span className="text-mute">· {tag.count}</span>
            </span>
          ))}
        </motion.div>
